...
private:
	...
	std::unordered_map<SoundID, SharedPtr<FMODAudioClip>> m_AudioClip{};
	...
	struct SoundQueue
	{
		SoundQueue(const SharedPtr<FMODAudioClip>& clip, Channel* channel)
			: aClip(clip)
			, aChannel(channel)
		{
		}
...
powe::SoundID powe::FMOD2DSound::FMODSoundImpl::RegisterSoundEntity(const std::string& filePath, bool shouldPreloaded)
{
	if (!m_PathCheck.contains(filePath))
	{
		...
		const SharedPtr<FMODAudioClip> clip{ std::make_shared<FMODAudioClip>(soundID,filePath) };
...
{
	const SoundID soundID{ m_AudioClipCnt++ };
	const SharedPtr<FMODAudioClip> audioClip{ std::make_shared<FMODAudioClip>(soundID,filePath) };
	SharedPtr<Channel> channel{};
