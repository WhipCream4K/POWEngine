...
void powe::SparseComponentManager::AddComponentToSparseSet(
	EntityID id, ComponentTypeID componentTypeId,
...
powe::RawByte* powe::SparseComponentManager::GetComponentData(EntityID id, ComponentTypeID compID,
	...
	bool callInternDestroy)
{
	...
	if(findItr != m_GameObjectToHandle.end() && findItr->second.contains(compID))
	{
		...
		if(handle < sparseSet.CurrentEmptyIndex)
		{
			...
			{
				const EntityID gameObject{ sparseSet.GameObjectIDs[i] };
...
void powe::SparseComponentManager::RemoveGameObjectFromSparse(const std::vector<EntityID>& gameObjects)
{
