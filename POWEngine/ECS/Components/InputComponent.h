...
namespace powe
{
	...
	public:
		...
		virtual void Execute(WorldEntity& worldEntity, float deltaTime, EntityID id,InputEvent inputEvent) = 0;
	};
	...
	public:
		...
		virtual void Execute(WorldEntity& worldEntity, float deltaTime, EntityID id, float axisValue) = 0;
	};
	...
	public:
		...
		void AddActionCommand(const std::string& name, const SharedPtr<ActionCommand>& actionCommand);
		...
		const std::unordered_map<std::string, std::vector<SharedPtr<ActionCommand>>> GetActionCommands() const { return m_ActionCommands; }
		const std::unordered_map<std::string, std::vector<SharedPtr<AxisCommand>>> GetAxisCommands() const { return m_AxisCommands; }

	private:

		std::unordered_map<std::string, std::vector<SharedPtr<ActionCommand>>> m_ActionCommands{};
