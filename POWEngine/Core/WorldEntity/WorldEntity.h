...
namespace powe
{
	...
	public:
		...
		void RegisterGameObject(EntityID id);

		EntityID GetNewEntityID();
		EntityID CreateNewEntity();

		bool IsGameObjectExist(EntityID id) const;
		...
		template<typename ComponentType>
		EnableIsBasedOf<BaseComponent, ComponentType, ComponentType*> GetComponent(EntityID id) const;


		const SparseComponentManager& GetSparseComponentManager() const { return m_SparseComponentManager; }

		template<typename Component>
		void RemoveComponentByType(EntityID id);

		void RemoveGameObject(EntityID id, bool removeRecord = true);
		...
		template<typename ComponentType>
		EnableIsBasedOf<BaseComponent, ComponentType, ComponentType*> FindUniqueComponent(powe::EntityID& ownerID) const;

		template<typename ComponentType>
		EnableIsBasedOf<BaseComponent, ComponentType, ComponentType*> FindUniqueComponent() const;

		SharedPtr<Archetype> GetArchetypeByGameObject(EntityID id) const;
		...
		bool GetGameObjectRecords(EntityID id, GameObjectRecord& outRecord) const;
		GameObjectRecord& GetRefGameObjectRecord(EntityID id);
	...
	private:
		...
		void RemoveComponentByID(EntityID id, ComponentTypeID componentID);
		...
		bool GetPreArchetypeTrait(EntityID id, PreArchetypeTrait& outTrait) const;

		// Actually trying to remove gameobjects from pending delete list
		void AddGameObjectToRecordRemoveList(EntityID id);
		void AddGameObjectToArchetypeRemoveList(const std::string& archetypeKey, EntityID id);
		void AddComponentToGameObjectRemoveList(EntityID id, ComponentTypeID componentTypeID);
		...
		void RemoveComponentFromPreArchetype(EntityID id, ComponentTypeID componentTypeId);
		void RemoveGameObjectFromPreArchetype(EntityID id);
	...
	private:
		...
		UniquePtr<SimpleThreadPool> m_SimpleThreadPool;
		...
		std::unordered_map<EntityID, std::vector<ComponentTypeID>> m_PendingDeleteComponentsFromGameObject;
		...
		std::unordered_map<EntityID, GameObjectRecord> m_GameObjectRecords;
		std::unordered_map<std::string, std::vector<EntityID>> m_PendingDeleteGameObjectsFromArchetype;
		std::vector<EntityID> m_PendingDeleteGameObjectsFromRecord;
	...
	template <typename ComponentType>
	EnableIsBasedOf<BaseComponent, ComponentType, ComponentType*> WorldEntity::AddComponentToGameObject(EntityID id,
...
#pragma endregion
	...
	template <typename ComponentType>
	EnableIsBasedOf<BaseComponent, ComponentType, ComponentType*> WorldEntity::GetComponent(EntityID id) const
	{
	...
	template <typename Component>
	void WorldEntity::RemoveComponentByType(EntityID id)
	{
	...
	template <typename ComponentType>
	EnableIsBasedOf<BaseComponent, ComponentType, ComponentType*> WorldEntity::FindUniqueComponent(powe::EntityID& ownerID) const
	{
